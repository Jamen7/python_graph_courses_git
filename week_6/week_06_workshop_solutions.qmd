---
title: "Workshop SOLUTIONS: Creating and Transforming Variables"
---
...
```{python}
# Calculate total physical activity
...
def total_pa_cat(total_pa_m_wk):
    if pd.isnull(total_pa_m_wk):
        return np.nan
    elif total_pa_m_wk < 500:
        return "low"
...
# Apply the function
pa["total_pa_cat"] = total_pa_cat(pa["total_pa_m_wk"])
```
...
```{python}
# Histogram of total physical activity
fig = px.histogram(pa, x='total_pa_m_wk', nbins=20, title='Distribution of Total Physical Activity (minutes per week)')
fig.show()
```
...
```{python}
# Boxplot of total physical activity by status
fig = px.box(pa, x='status', y='total_pa_m_wk', color='status', title='Total Physical Activity by Status')
fig.show()
```
