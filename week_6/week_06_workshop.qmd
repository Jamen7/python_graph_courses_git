...
pa = pa.drop(columns=['mvpa_s_day', "sedentary_ap_s_day", "light_ap_s_day"])
```
...
```{python}
def tot_pa_cat(tot_pa):
    if tot_pa < 500:
        return 'low'
    elif tot_pa >= 500 and tot_pa < 1000:
        return 'medium'
    else:
        return 'high'

tot_pa_catVec = np.vectorize(tot_pa_cat)
```

```{python}
tot_pa_cat(600)
pa['total_pa_cat'] = tot_pa_catVec(pa['total_pa_m_wk'])
...
```{python}
px.box(pa, x= 'total_pa_cat', y= 'bmi', color='total_pa_cat')
```

Write a short explanation of what you see in the plot.
**Answer** 
Those with low physical activity have the highest median of bmi at 28.1 with the smallest range of 24.7, while the medium physical activity group have the lowest median of 24.6 with the maximum bmi at 48. This suggests that the medium group has a positive (right) skew since some data points above the median are more spread out, similarly the high group has a positive stew.