...
```{python}
pd.concat([people,test_info_disordered], axis=1) # naive pasting data
```
...
```{python}
pd.concat([people,test_info_multiple], axis=1) # naive pasting data
```
...
## Practice q: jion employees and training sessions
...
Prediction: 3 rows and 4 columns
...
```{python}
pd.merge(tb_2019_africa,health_exp_2019) # we loss some African country with an inner-join 
```
...
```{python}
df = pd.merge(total_sales, product_feedback, how='inner')
df
```

Seven products are common on both datasets


```{python}
df['ratio_pos_unit'] = df['n_positive_reviews'] / df['total_units_sold']
df.sort_values(['ratio_pos_unit'], ascending=False)
```

The Desktop has the highest ratio of positive reviews to units sold but it should be **laptops** as per the notes. The calculation for the ratios above shows that the desktops have the highest ratio.
