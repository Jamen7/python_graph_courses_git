...
new_hiv_long = new_hiv_raw.melt(id_vars=['country', 'country_code'], value_name='new_cases', var_name='year')
new_hiv_long
```
...
total_hiv_long = total_hiv_raw.melt(id_vars=['country', 'country_code'], value_name='total_cases', var_name='year')
total_hiv_long
```
...
```{python}
# Your code here
population_2 = population.query('indicator =="SP.POP.TOTL"')
population_long = population_2.melt(id_vars=['country', 'indicator'], var_name= 'year', value_name= 'population')
...
```{python}
# Your code here
hiv_data_mini = hiv_data_combined_pop[hiv_data_combined_pop['country'].isin(["Nigeria", "Angola", "South Africa"])]
...
fig1 = px.line(
    ...
    labels={"incidence_rate": "Incidence Rate", "year": "Year"},
)
fig1.write_image('HIV_incidence_plot.png')
```
...
fig2 = px.line(
    ...
    labels={"prevalence_rate": "Prevalence Rate", "year": "Year"},
)
fig2.write_image('HIV_prevalence_plot.png')
```
...
hiv_data_combined_pop["continent"] = hiv_data_combined_pop["country"].apply(lambda x: coco.convert(names=x, to='continent'))
...
```{python}
hiv_data_combined_pop_2011 = hiv_data_combined_pop.query('year == 2011')
hiv_data_combined_pop_2011
```

```{python}
high_prevalence_rates_continent = hiv_data_combined_pop_2011.loc[hiv_data_combined_pop_2011.groupby("continent")["prevalence_rate"].idxmax()].sort_values(
    ["prevalence_rate"], ascending=False
) 
high_prevalence_rates_continent
```
...
fig3 = px.bar(
    ...
    x="country",
    y="prevalence_rate",
    title="Highest Prevalence Rates per Continent in 2011",
    labels={"prevalence_rate": "Prevalence Rate", "country": "Country per Continent"},
    text_auto=".0f",
)
fig3.write_image("Highest_prevalence_2011.png")
```