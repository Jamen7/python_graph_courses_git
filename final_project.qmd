...
highest_employ = (
    employ_child_fert.loc[
        employ_child_fert.groupby("continent")["employment_rate"].idxmax()
    ]
...
::: {.valuebox icon="mortarboard-fill" color="#4682B4" title="Highest Employment Rate"}
...
```{python}
fig_fert2 = px.bar(lowest20, x='Country Code', y='Fertility Rate', color= 'Continent', hover_name='Country/area', title='Lowest Total Fertility Rate in 2019').update_layout(showlegend=False)
...
fig3 = px.bar(
    ...
    color="Country/area",
    hover_name="Year",
    title="Highest Female Employment Rate in Each Continent",
#    labels={'mortality': 'Child Mortality', 'continent': 'Continent', 'country': 'Country'}
).update_layout(
    showlegend=False
)
#show(highest_employ[['Country/area', 'Continent','Year', 'Employment Rate']], caption="Highest Female Employment Rate in each Continent")
fig3.show()
```
...
continent_fig = px.box(
    ...
    showlegend=False, xaxis_title="Continent", yaxis_title="Employment Rate",
    sliders=[
        {
            "active": employ_child_fert['Year'].unique().tolist().index(2018),  # Set slider to 2018 index
        }
    ]
)
...
show(employ_child_fert, caption="Child mortality, total fertility and employment rate Dataset (1991 to 2019)", buttons=["csvHtml5"])
```
