...
```{python}
loans_clean = loans.drop(columns="rownames")
loans_clean
```
...
```{python}
demog_loan_info.to_csv('../outputs/demog_loan_information.csv', index=False)
```
...
```{python}
credit_vars = loans_clean.filter(regex='credit')
...
```{python}
#hi
high_income = loans_clean.query("annual_income > 100000")
high_income.to_csv("../outputs/high_earners.csv", index=False)
```
...
```{python}
high_risk = loans_clean.query('interest_rate > 15 | debt_to_income > 30 | delinq_2y >= 1')
high_risk.to_csv('../outputs/high_risk_loaners.csv', index=False)
```
...
```{python}
prime = loans_clean.query("(grade == 'A'| grade == 'B') & debt_to_income < 20 & delinq_2y == 0")
...
with pd.ExcelWriter('../outputs/loan_subsets.xlsx') as writer:
    demog_loan_info.to_excel(writer, sheet_name='Loan_Demog', index=False)
    credit_vars.to_excel(writer, sheet_name='Credit_Variables', index=False)
    high_income.to_excel(writer, sheet_name='High_Earners', index=False)
